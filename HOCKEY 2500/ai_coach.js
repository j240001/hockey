...
const AICoach = {
    ...
    mutateGenome: function(genome) {
        ...
        for (const role of roles) {
            ...
            // 2. Find every GOTO node in this role's behavior tree
            for (const seq of root.children) {
                const gotoNodes = seq.children.filter(n => n.type === "actGoToPosition");
                
                for (const targetNode of gotoNodes) {
                    const keys = ["minDistOwnGoal", "minDistOppGoal", "minDistLeftBoard", "minDistRightBoard"];
                    
                    // 3. Nudge every single key in the node
                    for (const key of keys) {
                        const delta = (Math.random() > 0.5 ? 20 : -20); // Fast 20px steps
                        let newVal = Number(targetNode[key] || 0) + delta;

                        // --- SAFETY CONSTRAINTS (Prevent Jitter) ---
                        if (key === "minDistOwnGoal" || key === "minDistOppGoal") {
                            newVal = Math.max(0, Math.min(newVal, 700)); // Rink length
                        }
                        if (key === "minDistLeftBoard" || key === "minDistRightBoard") {
                            const otherK = (key === "minDistLeftBoard") ? "minDistRightBoard" : "minDistLeftBoard";
                            const otherV = Number(targetNode[otherK] || 0);
                            
                            // Width limit: Rink is 340. Keep 40px corridor
                            if (newVal + otherV > 300) newVal = 300 - otherV; 
                            newVal = Math.max(0, newVal);
                        }

                        targetNode[key] = newVal;
                        totalAdjusted++;
                    }
                }
            }
        }
        this.mutationDetails = `Global Overhaul: Adjusted ${totalAdjusted} parameters.`;
    },

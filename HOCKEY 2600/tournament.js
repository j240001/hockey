...
startNextMatch: function() {
        ...
        // --- THE "W" KEY TOGGLE LOGIC ---
        const isMyTeamPlaying = (Team0_Strategy.code === this.targetTeam || Team1_Strategy.code === this.targetTeam);

        // Only toggle watchMode if the feature is switched ON
        if (this.autoWatchTarget && isMyTeamPlaying) {
            this.watchMode = true;   
            this.speedMult = 1;      
        } else {
        ...
        if (!this.isTrainingEpisode) {
            console.log(`⚔️ MATCH ${this.currentMatchIndex + 1}: ${Team0_Strategy.code} vs ${Team1_Strategy.code}`);
        }
        
        fullGameReset(); 
        this.simLoop(); // Always use the original simLoop
    },
    ...
    recordResult: function(isShootout) {
        ...
        const broadcastPacket = {
            ...
            currentIndex: this.currentMatchIndex, // ADD THIS
            totalMatches: this.matches.length,    // ADD THIS
            lastGame: {
                team0: Strategies[this.matches[this.currentMatchIndex-1].home].code,
                team1: Strategies[this.matches[this.currentMatchIndex-1].away].code,
                score0: scoreTeam0,
                score1: scoreTeam1,
                isOT: (currentPeriod > 3)
            }
        };

        // BROADCAST: Update the second monitor
        localStorage.setItem('TOURNAMENT_STANDINGS_DATA', JSON.stringify(broadcastPacket));

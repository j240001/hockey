...
const RINK_W = 800;
const RINK_H = 340;
const RINK_X = 100; // Hardcoded: (1000 - 800) / 2
const RINK_Y = 150;
...
const DOTS = {
    ...
    // NEUTRAL ZONE (Outside Blue Lines)
    // Your Blue Lines are at RX +/- 110. Dots should be approx +/- 135
    NZ_TOP_LEFT:    { x: RX - 91, y: RY - 89 },
    NZ_BOT_LEFT:    { x: RX - 91, y: RY + 89 },
    NZ_TOP_RIGHT:   { x: RX + 91, y: RY - 89 },
    NZ_BOT_RIGHT:   { x: RX + 91, y: RY + 89 },

    // END ZONES (Inside Circles)
    // Goals are at RX +/- 325. Dots usually ~20-30px from goal line?
    // Let's estimate them at +/- 265
    EZ_TOP_LEFT:    { x: RX - 258, y: RY - 89 }, // Defending Zone Team 0
    EZ_BOT_LEFT:    { x: RX - 258, y: RY + 89 },
    EZ_TOP_RIGHT:   { x: RX + 258, y: RY - 89 }, // Defending Zone Team 1
    EZ_BOT_RIGHT:   { x: RX + 258, y: RY + 89 }
};

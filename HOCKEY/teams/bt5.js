...
// ==========================================
(function() {
    const STRATEGY_ID = "BT5_CANUCKS";
    const STRATEGY_NAME = "BT5_CANUCKS";
    ...
    const TREE_ATTACKER = 
        new SelectorNode([
            ...
            const finalY = (bb.p.y < RY) ? carrier.y - 120 : carrier.y + 120;
    ...
    const TREE_WINGER = 
        new SelectorNode([
            ...
            const carrier = getPlayerById(puck.ownerId);
            if (!carrier) return { tx: bb.p.x, ty: bb.p.y, action:'none' };
            const finalX = carrier.x + (bb.forwardDir * -10);
            const finalY = (bb.p.y < RY) ? carrier.y - 120 : carrier.y + 120;
            return { tx: finalX, ty: finalY, action:'none' };
        })
            ]),
            ...
            new SequenceNode([
                ...
                condPuckInNeuZone,
                new ActionNode(bb => ({
            tx: bb.myGoalX + (bb.forwardDir * (300)),
            ty: RY,
            action:'none'
        }))
            ]),
    ...
    const TREE_DEFENDER = 
        new SelectorNode([
            new SequenceNode([
                ...
                new SelectorNode([
                    ...
                    new SequenceNode([
                        condPuckInNeuZone,
                        new ActionNode(bb => ({
            tx: bb.myGoalX + (bb.forwardDir * (180)),
            ty: RY,
            action:'none'
        }))
                    ]),
            ...
            const carrier = getPlayerById(puck.ownerId);
            if (!carrier) return { tx: bb.p.x, ty: bb.p.y, action:'none' };
            const finalX = carrier.x + (bb.forwardDir * -100);
            ...
            const carrier = getPlayerById(puck.ownerId);
            if (!carrier) return { tx: bb.p.x, ty: bb.p.y, action:'none' };
            const finalX = carrier.x + (bb.forwardDir * -90);
            const finalY = (bb.p.y < RY) ? carrier.y - 0 : carrier.y + 0;
            return { tx: finalX, ty: finalY, action:'none' };
        })
                    ]),

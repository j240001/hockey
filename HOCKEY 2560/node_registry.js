...
const NODE_REGISTRY = {
    ...
    "condPatienceBreached": {
        cat: "cond", label: "Patience Breached?", acr: "THREAT?", 
        params: { dist: 80 },
        generate: (d) => `new ConditionNode(bb => (bb.p.carrier_patience_dist || 9999) < ${d.dist})`,
        execute: (d) => new ConditionNode(bb => (bb.p.carrier_patience_dist || 9999) < d.dist)
    },

    "condInShotRange": {
        cat: "cond", label: "In Shooting Range?", acr: "SR?", req: "ANY", params: { dist: 250 },
        generate: (data) => `new ConditionNode(bb => {let d=${data.dist}||250; return Math.hypot(bb.enemyGoal-bb.p.x, (typeof RY!=='undefined'?RY:300)-bb.p.y) < d;})`,
        execute: (p, params) => new ConditionNode(bb => {let d=(params&&params.dist)?params.dist:250; return Math.hypot(bb.enemyGoal-bb.p.x, (typeof RY!=='undefined'?RY:300)-bb.p.y) < d;})
    },
    ...
    "condAmIClosest": {
        cat: "cond", label: "Am I Closest?", acr: "ME?", req: "ANY", params: { dist: 0 },
        generate: (data) => `new ConditionNode(bb => {let myD=Math.hypot(bb.p.x-puck.x,bb.p.y-puck.y); if(${data.dist}>0 && myD<${data.dist}) return true; let c=null,d=9999; for(let m of players){if(m.team===bb.p.team && m.type==='skater'){let dist=Math.hypot(m.x-puck.x,m.y-puck.y);if(dist<d){d=dist;c=m;}}}return c&&c.id===bb.p.id;})`,
        execute: (p, params) => new ConditionNode(bb => {let myD=Math.hypot(bb.p.x-puck.x,bb.p.y-puck.y); let t=params&&params.dist?params.dist:0; if(t>0 && myD<t) return true; let c=null,d=9999; for(let m of players){if(m.team===bb.p.team && m.type==='skater'){let dist=Math.hypot(m.x-puck.x,m.y-puck.y);if(dist<d){d=dist;c=m;}}}return c&&c.id===bb.p.id;})
    },
    ...
    "condHasBreakoutPass": {
        cat: "cond", label: "Qualified Breakout", acr: "BRK?", req: "ANY", params: { safety: 60, fwd: 150, back: 10 },
        generate: (d) => `new ConditionNode(bb => { try { const t = (typeof findQualifiedBreakout === 'function' ? findQualifiedBreakout(bb.p, bb, {safety:${d.safety},fwd:${d.fwd},back:${d.back}}) : null); if(t){bb.passTarget=t; return true;} return false; } catch(e){return false;} })`,
        execute: (p, params) => new ConditionNode(bb => { try { const t = (typeof findQualifiedBreakout === 'function' ? findQualifiedBreakout(bb.p, bb, params) : null); if(t){bb.passTarget=t; return true;} return false; } catch(e){return false;} })
    },
    ...
    "condHasBackdoor": {
        cat: "cond", label: "Has Backdoor", acr: "BD?", req: "ANY", params: { dist: 100, open: 50, goal: 60 },
        generate: (d) => `new ConditionNode(bb => { try { const t = (typeof findBackdoorTarget === 'function' ? findBackdoorTarget(bb.p, ${d.dist}, ${d.open}, ${d.goal}) : null); if(t && Math.abs(t.x) > 110){bb.passTarget=t; return true;} return false; } catch(e){return false;} })`,
        execute: (p, params) => new ConditionNode(bb => { try { const t = (typeof findBackdoorTarget === 'function' ? findBackdoorTarget(bb.p, params.dist, params.open, params.goal) : null); if(t && Math.abs(t.x) > 110){bb.passTarget=t; return true;} return false; } catch(e){return false;} })
    },
    ...
    "actIntelTest": {
        cat: "act", label: "Intel Test", acr: "SCAN",
        generate: () => `new ActionNode(bb => intel_test(bb), "Intel Scan")`,
        // Missing execute added here:
        execute: () => new ActionNode(bb => intel_test(bb), "Intel Scan")
    },
    ...
    "actCarrierIntel": {
        ...
        req: "ANY",
        params: { isForward: 1 }, 
        generate: (d) => `new ActionNode(bb => carrier_intel(bb, ${d.isForward}), "Carrier Intel")`,
        // Missing execute added here:
        execute: (d) => new ActionNode(bb => carrier_intel(bb, d.isForward), "Carrier Intel")
    },
    ...
    "actPassTest": { 
        ...
        acr: "PASS", 
        req: "ANY", 
        params: { minOpenness: 40 }, 
        generate: (d) => `new ActionNode(bb => pass_test(bb, ${d.minOpenness}), "Pass Test")`,
        execute: (p, d) => new ActionNode(bb => pass_test(bb, d.minOpenness), "Pass Test") 
    },
    ...
    "actTacticalKeepAwayPass": {
        cat: "act", label: "Tactical Keep Away", acr: "T_KEEP", req: "ANY", params: { minVacuum: 0 },
        generate: (d) => `new ActionNode(bb => { try { const t = (typeof executeTacticalKeepAway === 'function' ? executeTacticalKeepAway(bb.p, bb, {minVacuum:${d.minVacuum}}) : null); if(t){ return t; } return "FAILURE"; } catch(e){return "FAILURE";} }, "Tactical Keep Away")`,
        execute: (p, params) => new ActionNode(bb => { try { const t = (typeof executeTacticalKeepAway === 'function' ? executeTacticalKeepAway(bb.p, bb, params) : null); if(t){ return t; } return "FAILURE"; } catch(e){return "FAILURE";} }, "Tactical Keep Away")
    },

    "actDynamicCarry": { 
        cat: "act", label: "Dynamic Carry", acr: "WEAVE", req: "ANY",
        generate: () => `new ActionNode(bb => getDynamicCarry(bb.p, bb), "Dynamic Carry")`,
        execute: (p) => new ActionNode(bb => getDynamicCarry(bb.p, bb), "Dynamic Carry")
    },

    "actShoot": {
        cat: "act", label: "Shoot", acr: "SHOT", req: "ANY",
        generate: () => `new ActionNode(bb => ({ tx: bb.enemyGoal, ty: (typeof RY !== 'undefined' ? RY : 300), action: "shoot" }), "Shoot")`,
        execute: () => new ActionNode(bb => ({ tx: bb.enemyGoal, ty: (typeof RY !== 'undefined' ? RY : 300), action: "shoot" }), "Shoot")
    },
    "actDriveNet": {
        cat: "act", label: "Drive Net", acr: "DRIV", req: "ANY",
        generate: () => `new ActionNode(bb => ({ tx: bb.enemyGoal - (bb.forwardDir * 40), ty: (typeof RY !== 'undefined' ? RY : 300), action: "none" }), "Drive Net")`,
        execute: () => new ActionNode(bb => ({ tx: bb.enemyGoal - (bb.forwardDir * 40), ty: (typeof RY !== 'undefined' ? RY : 300), action: "none" }), "Drive Net")
    },

    "actLaneCarry": {
        cat: "act", label: "Lane Carry", acr: "L-CARY", req: "ANY",
        params: { forwardDist: 150 },
        generate: (d) => `new ActionNode(bb => ({ tx: bb.p.x + (bb.forwardDir * ${d.forwardDist}), ty: bb.p.y, action: "none" }), "Lane Carry")`,
        execute: (p, d) => new ActionNode(bb => ({ tx: bb.p.x + (bb.forwardDir * d.forwardDist), ty: bb.p.y, action: "none" }), "Lane Carry")
    },

    "actExecuteCarry": {
        cat: "act", label: "Execute Carry", acr: "CARY", req: "ANY",
        generate: () => `new ActionNode(bb => { if(bb.carryTarget)return{tx:bb.carryTarget.x,ty:bb.carryTarget.y,action:"none"}; return {tx:bb.enemyGoal,ty:(typeof RY !== 'undefined' ? RY : 300),action:"none"}; }, "Carry")`,
        execute: () => new ActionNode(bb => { if(bb.carryTarget)return{tx:bb.carryTarget.x,ty:bb.carryTarget.y,action:"none"}; return {tx:bb.enemyGoal,ty:(typeof RY !== 'undefined' ? RY : 300),action:"none"}; }, "Carry")
    },
    "actExecutePass": {
        cat: "act", label: "Execute Pass", acr: "PASS", req: "ANY",
        generate: () => `new ActionNode(bb => { if(bb.passTarget)return{tx:bb.passTarget.x,ty:bb.passTarget.y,action:"pass",target:bb.passTarget}; return {tx:bb.enemyGoal,ty:(typeof RY !== 'undefined' ? RY : 300),action:"none"}; }, "Pass")`,
        execute: () => new ActionNode(bb => { if(bb.passTarget)return{tx:bb.passTarget.x,ty:bb.passTarget.y,action:"pass",target:bb.passTarget}; return {tx:bb.enemyGoal,ty:(typeof RY !== 'undefined' ? RY : 300),action:"none"}; }, "Pass")
    },
    "actSmartPass": {
        cat: "act", label: "Exec Smart Pass", acr: "DO_PASS", req: "ANY",
        generate: () => `new ActionNode(bb => { try { if(bb.passTarget)return{tx:bb.passTarget.x,ty:bb.passTarget.y,action:"pass",target:bb.passTarget}; return null; } catch(e){return null;} }, "smartpass")`,
        execute: () => new ActionNode(bb => { try { if(bb.passTarget)return{tx:bb.passTarget.x,ty:bb.passTarget.y,action:"pass",target:bb.passTarget}; return null; } catch(e){return null;} }, "smartpass")
    },
    "actGoBackdoor": {
    ...
    "actEvadePressure": {
        ...
        generate: () => `new ActionNode(bb => { try { return (typeof evadePressure === 'function' ? evadePressure(bb) : {tx:bb.p.x,ty:bb.p.y,action:'none'}); } catch(e){return {tx:bb.p.x,ty:bb.p.y,action:'none'};} }, "Evade")`,
        execute: () => new ActionNode(bb => { try { return (typeof evadePressure === 'function' ? evadePressure(bb) : {tx:bb.p.x,ty:bb.p.y,action:'none'}); } catch(e){return {tx:bb.p.x,ty:bb.p.y,action:'none'};} }, "Evade")
    },
    ...
    "actSmartShoot": { 
        ...
        label: "Smart Shot", 
        acr: "SSHOT", 
        params: { laneWidth: 15, angleBuf: 10 },
        generate: (d) => `new ActionNode(bb => getSmartShootTarget(bb.p, bb, ${d.laneWidth}, ${d.angleBuf}), "smartshoot")`,
        execute: (p, d) => new ActionNode(bb => getSmartShootTarget(bb.p, bb, d.laneWidth, d.angleBuf), "smartshoot")
    },

    "actOzoneReliefPass": {
        cat: "act", label: "Ozone Relief Pass", acr: "RELIEF", req: "ANY",
        // FIX: Check for null and return "FAILURE" string
        generate: () => `new ActionNode(bb => { try { const t = (typeof findLeastGuardedInZone === 'function' ? findLeastGuardedInZone(bb.p, bb) : null); if(t){ return {tx:t.x, ty:t.y, action:"pass", target:t}; } return "FAILURE"; } catch(e){return "FAILURE";} }, "OZONE R PASS")`,
        execute: () => new ActionNode(bb => { try { const t = (typeof findLeastGuardedInZone === 'function' ? findLeastGuardedInZone(bb.p, bb) : null); if(t){ return {tx:t.x, ty:t.y, action:"pass", target:t}; } return "FAILURE"; } catch(e){return "FAILURE";} }, "OZONE R PASS")
    },

    // --- ACTIONS: DEFENSE ---

    "actDzoneReliefPass": {
    cat: "act", label: "D-Zone Relief Pass", acr: "D_RELIEF", req: "ANY",
    generate: () => `new ActionNode(bb => { try { const t = (typeof findDzoneReliefTarget === 'function' ? findDzoneReliefTarget(bb.p, bb) : null); if(t){ return {tx:t.x, ty:t.y, action:"pass", target:t}; } return "FAILURE"; } catch(e){return "FAILURE";} }, "D-Zone Relief")`,
    execute: () => new ActionNode(bb => { try { const t = (typeof findDzoneReliefTarget === 'function' ? findDzoneReliefTarget(bb.p, bb) : null); if(t){ return {tx:t.x, ty:t.y, action:"pass", target:t}; } return "FAILURE"; } catch(e){return "FAILURE";} }, "D-Zone Relief")
    },
    ...
    "actSmartIntercept": {
    cat: "act", label: "Smart Intercept", acr: "INT", params: { minSpeed: 1.5 },
    generate: (d) => `new ActionNode(bb => { try { const t = (typeof getPuckIntercept === 'function') ? getPuckIntercept(bb.p, ${d.minSpeed}) : { x: puck.x, y: puck.y }; return { tx: t.x, ty: t.y, action: "none" }; } catch(e) { return { tx: bb.p.x, ty: bb.p.y, action: "none" }; } }, "Intercept")`,
    // ADDED LABEL ARGUMENT HERE
    execute: (p, d) => new ActionNode(bb => { const t = getPuckIntercept(bb.p, d.minSpeed); return { tx: t.x, ty: t.y, action: "none" }; }, "Intercept")
    },
    ...
    "actGoToPosition": {
        ...
        generate: (d) => `new ActionNode(bb => { try { return (typeof getPositionWithinLimits === 'function' ? getPositionWithinLimits(bb.p, bb, ${JSON.stringify(d)}) : {tx:bb.p.x,ty:bb.p.y,action:'none'}); } catch(e){return {tx:bb.p.x,ty:bb.p.y,action:'none'};} }, "Go To Pos")`,
        execute: (d) => new ActionNode(bb => { try { return (typeof getPositionWithinLimits === 'function' ? getPositionWithinLimits(bb.p, bb, d) : {tx:bb.p.x,ty:bb.p.y,action:'none'}); } catch(e){return {tx:bb.p.x,ty:bb.p.y,action:'none'};} }, "Go To Pos")
    },
};
...
const PALETTE_LAYOUT = [
    ...
    { header: "STATE SETTERS" }, "actIntelTest", "actCarrierIntel", "actPassTest", "condHasPuck", "condTeamHasPuck", "condOppHasPuck", "condLoosePuck",
    { header: "MATRIX / FORMATION" }, "condPuckInZone", "actFormationTarget", "actHoverDynamic", "actGoToPosition",
    { header: "OFFENSIVE" }, "actTacticalKeepAwayPass", "actDynamicCarry", "condPatienceBreached", "condInShotRange", "actShoot", "actSmartShoot", "actOzoneReliefPass", "actDriveNet", "condHasBackdoor", "actGoBackdoor", "condSmartPass", "actSmartPass", "condWeightedPassCheck", "actLaneCarry", "actExecuteCarry", "actExecutePass", "actEvadePressure",
    { header: "DEFENSIVE" }, "actDzoneReliefPass", "actDefendHome", "actAggressiveGap", "actClearPuck", "actSupportPosition", "condIsLastMan", "actLastManSafety",
